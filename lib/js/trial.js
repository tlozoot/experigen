...
Experigen.make_into_trial = function (that) {
	...
	that.advance = function(spec) {
		...
		if (Experigen.screen().callingPart===0) { 
		...
		if (Experigen.screen().callingPart===Experigen.screen().currentPart) {
			...
			if (Experigen.screen().currentPart > Experigen.screen().parts.length) {
				...
				$(part).find(':input[type!="hidden"]').first().focus();
			}
		}
	}
	...
	that.continueButton = function (str) {
		if (!str) {
			str = Experigen.settings.strings.continueButton;
		}
		
		return '<input type="button" value="' + str + '" onClick="Experigen.screen().continueButtonClick(this);">'
	}

	that.continueButtonClick = function (caller) {
